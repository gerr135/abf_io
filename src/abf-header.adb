...
procedure PrintPrincialParams(abfH: in ABFFileHeader;
  ...
  begin
	...
	New_Line(File);
	Put(File,"cnahhels:     ");
		for i in 0 .. ABF_ADCCOUNT-1 loop Put(File,i,Width=>8); end loop;
		New_Line(File);
...
function  Read_Header  (FileName: String) return ABFFileHeader is
  ...
  begin
	...
	then
-- 		Put("file sig:     ");Put_Line(abfH.lFileSignature'Img);
-- 		Put("header size:  ");Put_Line(abfH.lHeaderSize'Img);
-- 		Put("acq mode:     ");Put_Line(abfH.nOperationMode'Img);
-- 		Put("num episodes: ");Put_Line(abfH.lActualEpisodes'Img);
-- 		Put("2nd sample:   ");Put_Line(abfH.fADCSecondSampleInterval'Img);
